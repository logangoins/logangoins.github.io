...
We find that there's no unique SMB shares the user "operator" has access to, meaning MSSQL
...
After authentication, looking through the various non-default databases yields no results. No password hashes that we can crack for further access.
...
Because "raven" has full configuration permissions of the Domain, we can abuse the ESC7 vulnerability, as part of the ESC family of ADCS vulnerabilities. 
